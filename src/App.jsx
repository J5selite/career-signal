...
function CardBack({glow,sz=1.05}){
  ...
  return(
    <div style={{width:w,height:h,borderRadius:8,background:"#0b0b0b",border:"1px solid #333",boxShadow:`0 0 50px 12px ${glow},0 0 120px 36px ${glow},0 0 220px 60px ${glow},0 6px 18px #00000099`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:12,position:"relative",overflow:"hidden",animation:"pulseGlow 1.6s ease-in-out infinite",fontFamily:"'Bebas Neue',sans-serif",userSelect:"none"}}>
      ...
      <div style={{color:"#ffffff22",fontSize:8,letterSpacing:2,fontFamily:"'Bebas Neue',sans-serif"}}>TAP TO REVEAL</div>
    </div>
  );
}
...
function ShareCard({card,onClose}){
  ...
  const download=async()=>{
    ...
    try{
      ...
      a.href=url;a.click();
      try{window.__csTrack?.("card_shared");}catch{}
        ...
        <div id="share-card-inner" style={{background:t.bg,border:`1px solid ${t.acc}44`,borderRadius:12,padding:"28px 24px",fontFamily:"'Bebas Neue',sans-serif",boxShadow:`0 0 40px ${t.glow}`}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
            <div>
              ...
              <div style={{color:"#ffffff55",fontSize:11,letterSpacing:1,marginTop:2,fontFamily:"'Bebas Neue',sans-serif"}}>{card.uni}</div>
            </div>
          ...
          <div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap"}}>
            {card.profile_type&&<div style={{background:`${t.acc}14`,border:`1px solid ${t.acc}33`,borderRadius:3,padding:"3px 8px",color:t.acc,fontSize:7,letterSpacing:1,textTransform:"uppercase",opacity:0.8,fontFamily:"'Bebas Neue',sans-serif"}}>{card.profile_type}</div>}
            {card.archetype&&<div style={{background:`${t.acc}14`,border:`1px solid ${t.acc}33`,borderRadius:3,padding:"3px 8px",color:t.acc,fontSize:7,letterSpacing:1,textTransform:"uppercase",opacity:0.8,fontFamily:"'Bebas Neue',sans-serif"}}>{card.archetype}</div>}
          </div>
          {/* Stat bars */}
            ...
            {STATS.map(st=>{
              ...
              return(
                <div key={st} style={{display:"flex",alignItems:"center",gap:8}}>
                  <span style={{color:"#ffffff33",fontSize:8,minWidth:34,letterSpacing:0.5,fontFamily:"'Bebas Neue',sans-serif"}}>{st}</span>
          ...
          {/* Thesis one-liner */}
          {thesis1&&<div style={{color:"#ffffff44",fontSize:9,lineHeight:1.6,marginBottom:14,fontFamily:"'Bebas Neue',sans-serif",fontStyle:"italic",borderLeft:`2px solid ${t.acc}33`,paddingLeft:10}}>{thesis1}</div>}
          {/* Footer */}
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",borderTop:`1px solid ${t.acc}22`,paddingTop:12}}>
            <div style={{color:"#ffffff22",fontSize:7,letterSpacing:1,fontFamily:"'Bebas Neue',sans-serif"}}>based on visible profile evidence only</div>
            <div style={{color:t.acc,fontSize:9,letterSpacing:2,opacity:0.5}}>CAREER SIGNAL</div>
          </div>
        </div>
        <div style={{display:"flex",gap:8,marginTop:14,justifyContent:"center"}}>
          <button onClick={download} disabled={saving} style={{background:"var(--gold)",color:"var(--gold-ink)",border:"none",padding:"9px 22px",borderRadius:5,cursor:saving?"wait":"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,fontWeight:700,letterSpacing:2,textTransform:"uppercase",opacity:saving?0.6:1}}>{saving?"RENDERING…":"⬇ DOWNLOAD PNG"}</button>
        </div>
        <div style={{color:saveErr?"#ff4444":"var(--v333)",fontSize:8,letterSpacing:1,fontFamily:"'Bebas Neue',sans-serif",textAlign:"center",marginTop:8}}>{saveErr||"saves a 2× image ready for stories & group chats"}</div>
        <button onClick={onClose} style={{display:"block",margin:"8px auto 0",background:"none",border:"none",color:"var(--v333)",cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,letterSpacing:2,textTransform:"uppercase"}}>CLOSE</button>
      </div>
    </div>
  );
  ...
  return(
    <svg width={size} height={size} style={{display:"block",overflow:"visible"}}>
      ...
      {STATS.map((s,i)=>{
        ...
        return <text key={s} x={lx} y={ly+3} textAnchor="middle" fill="var(--v555)" fontSize="9" fontFamily="'Bebas Neue',sans-serif">{s}</text>;
      })}
    </svg>
  );
}
...
function Trend({card,acc}){
  ...
  return(
    <svg width={W} height={H} style={{display:"block",overflow:"visible"}}>
      <text x={W/2} y={8} textAnchor="middle" fill="var(--v444)" fontSize="7" fontFamily="'Bebas Neue',sans-serif" letterSpacing="1">OVR OVER SCANS</text>
      ...
      {pts.map((p,i)=>(
        <g key={i}>
          ...
          <text x={X(i)} y={H-4} textAnchor="middle" fill="var(--v444)" fontSize="7" fontFamily="'Bebas Neue',sans-serif">{p.d?new Date(p.d).toLocaleDateString(undefined,{month:"short",day:"numeric"}):""}</text>
        </g>
      ))}
    </svg>
  );
...
function Key({label,sub,wide,pressed}){
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minWidth:wide?76:46,height:46,background:pressed?"var(--gold)":"var(--s16)",border:`1px solid ${pressed?"var(--gold)":"var(--v2a)"}`,borderBottom:`${pressed?"1px":"3px"} solid ${pressed?"var(--gold-deep)":"var(--v333)"}`,borderRadius:5,padding:"4px 8px",transform:pressed?"translateY(2px)":"none",transition:"all 0.12s",boxShadow:pressed?"none":"0 2px 0 var(--gold-ink)",cursor:"default"}}>
      <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:10,fontWeight:700,color:pressed?"var(--gold-ink)":"var(--vaaa)",letterSpacing:0.5,textAlign:"center",lineHeight:1.2}}>{label}</span>
      {sub&&<span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:7,color:pressed?"#00000077":"var(--v444)",letterSpacing:0.5,marginTop:2}}>{sub}</span>}
    </div>
  );
}
...
export default function App(){
  ...
  // Each view becomes a distinct SPA "page" for analytics: Vercel's script
  // watches History API calls, so this hash swap registers a pageview per tab.
  useEffect(()=>{
  ...
  const analyse=async(forceUpdate=false)=>{
    ...
    try{
      ...
      track(updating?"card_updated":"card_created",{ovr:OVR,screenshots:imgs.length});
    }catch(e){setErr(friendlyErr(e.message));console.error(e);track("scan_failed",{error:String(e.message).slice(0,80)});}
...
${JSON.stringify({name:sel.name,uni:sel.uni,uni_years:sel.uni_years,cohort:sel.year,company:sel.company,role:sel.role,how:sel.how,prior_roles:sel.prev,grades:sel.grades,timeline:sel.timeline,evidence:sel.evidence,notes:sel.notes,stats:sel.stats,OVR:sel.OVR,stat_reasons:sel.stat_reasons,confidence:sel.confidence,thesis:sel.thesis,best_signal:sel.best_signal,weak_signal:sel.weak_signal,not_proven:sel.not_proven,peer_calibration:sel.peer_calibration,floor:sel.floor,base_case:sel.base_case,ceiling:sel.ceiling,upgrade:sel.upgrade,improvement_plan:sel.improvement_plan,tier_path:sel.tier_path,ninety:sel.ninety?{OVR:sel.ninety.OVR,summary:sel.ninety.summary}:null})}${knowledge.trim()?`\n\nSCOUT KNOWLEDGE (trusted user calibration context): ${knowledge.trim()}`:""}${rubric.trim()?`\n\nOWNER RUBRIC (owner taste calibration — informs judgment, evidence still wins): ${rubric.trim()}`:""}`;
  ...
  const rescore=async()=>{
    ...
    try{
      ...
      track("card_rescored",{delta:newCard.OVR-sel.OVR});
...
Marked NOT ELIGIBLE (never re-propose these or variants): ${JSON.stringify(kept.filter(i=>i.status==="na").map(i=>i.t))}`;
  ...
  const rateHowGood=async()=>{
    ...
    try{
      ...
      track("howgood_rated",{grade:sc.grade});
  ...
  return(
    <div style={{minHeight:"100vh",background:"var(--bg)",color:"var(--veee)",fontFamily:"'Bebas Neue',sans-serif"}}>
      <style>{`
      ...
      <div style={{display:"flex",alignItems:"center",borderBottom:"1px solid var(--s11)",background:"var(--bg)",padding:"0 28px",position:"sticky",top:0,zIndex:100}}>
        ...
        {[["home","home"],["create","create"],["howgood","how good?"],["versus","versus"],["leaderboard","leaderboard"],["guide","guide"],["changelog","changelog"]].map(([v,label])=>(
          <button key={v} className="ghost" onClick={()=>{setView(v);reset();setSel(null);}} style={{background:"none",border:"none",borderBottom:view===v?"2px solid var(--gold)":"2px solid transparent",cursor:"pointer",padding:"15px 14px",color:view===v?"var(--gold)":"var(--v444)",fontFamily:"'Bebas Neue'",fontSize:10,letterSpacing:2,textTransform:"uppercase",transition:"color 0.15s",whiteSpace:"nowrap"}}>{label}</button>
        ))}
        {myId&&cards.some(c=>c.id===myId)&&(
          <button className="ghost" onClick={()=>{const me=cards.find(c=>c.id===myId);if(me){setSel(me);setView("profile");reset();}}} style={{background:"none",border:"none",borderBottom:"2px solid transparent",cursor:"pointer",padding:"15px 14px",color:"var(--gold)",fontFamily:"'Bebas Neue'",fontSize:10,letterSpacing:2,textTransform:"uppercase"}}>★ MY CARD</button>
        )}
        <div style={{flex:1}}/>
        <button onClick={exportCards} disabled={!cards.length} title="One-click backup — downloads your whole collection as JSON" style={{background:"none",border:"1px solid var(--v1e)",color:cards.length?"var(--gold)":"var(--v1e)",borderRadius:5,padding:"4px 11px",cursor:cards.length?"pointer":"not-allowed",fontFamily:"'Bebas Neue',sans-serif",fontSize:11,lineHeight:1.4,marginRight:8}}>⬇</button>
        <button onClick={()=>setTheme(t=>t==="dark"?"light":"dark")} title={theme==="dark"?"Switch to light mode":"Switch to dark mode"} style={{background:"none",border:"1px solid var(--v1e)",color:"var(--v444)",borderRadius:5,padding:"4px 11px",cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:11,lineHeight:1.4,marginRight:8,transition:"color 0.15s,border-color 0.15s"}} onMouseEnter={e=>{e.target.style.color="var(--gold)";e.target.style.borderColor="var(--gold)";}} onMouseLeave={e=>{e.target.style.color="var(--v444)";e.target.style.borderColor="var(--v1e)";}}>{theme==="dark"?"☀":"☾"}</button>
        <button onClick={()=>{setView("settings");reset();setSel(null);}} title="Settings" style={{background:"none",border:"1px solid var(--v1e)",color:view==="settings"?"var(--gold)":"var(--v444)",borderRadius:5,padding:"4px 11px",cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:11,lineHeight:1.4,marginRight:14,transition:"color 0.15s,border-color 0.15s"}} onMouseEnter={e=>{e.target.style.color="var(--gold)";e.target.style.borderColor="var(--gold)";}} onMouseLeave={e=>{e.target.style.color=view==="settings"?"var(--gold)":"var(--v444)";e.target.style.borderColor="var(--v1e)";}}>⚙</button>
      ...
      <div style={{maxWidth:880,margin:"0 auto",padding:"36px 24px"}}>
        ...
        {lightbox&&(()=>{
          ...
          <div onClick={()=>setLightbox(null)} onTouchStart={e=>{touchX=e.touches[0].clientX;}} onTouchEnd={e=>{if(touchX===null||L.list.length<2)return;const dx=e.changedTouches[0].clientX-touchX;if(Math.abs(dx)>48){go(dx<0?1:-1);}touchX=null;}} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.93)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:"20px 56px"}}>
            ...
            {L.del&&<button onClick={e=>{e.stopPropagation();L.del(L.idx);const nl=L.list.filter((_,i)=>i!==L.idx);nl.length?setLightbox({...L,list:nl,idx:Math.min(L.idx,nl.length-1)}):setLightbox(null);}} style={{position:"fixed",top:14,right:14,background:"#dc2626",border:"none",color:"#fff",padding:"9px 16px",borderRadius:6,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",boxShadow:"0 2px 12px rgba(220,38,38,0.5)"}}>✕ DELETE THIS SCREENSHOT</button>}
            <div style={{position:"fixed",bottom:14,left:0,right:0,textAlign:"center",color:"#ffffffaa",fontSize:9,letterSpacing:2,fontFamily:"'Bebas Neue',sans-serif",textTransform:"uppercase"}}>
                ...
                <div style={{display:"flex",gap:16,flexWrap:"wrap",justifyContent:"center",alignItems:"flex-start",marginBottom:40}}>
                  {sorted.slice(0,Math.min(3,sorted.length)).map(c=>(
                    <div key={c.id} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
                      ...
                      {c.OVR<80&&<div style={{background:"var(--s0c)",border:"1px dashed var(--v1e)",borderRadius:4,padding:"4px 10px",color:"var(--v444)",fontSize:8,letterSpacing:1.5,textTransform:"uppercase",fontFamily:"'Bebas Neue',sans-serif"}}>⏳ PENDING MORE CARDS · below 80 OVR</div>}
                    </div>
                  ))}
                </div>
            ...
            {cards.length===0&&(
              <div style={{textAlign:"center",padding:"60px 0"}}>
                ...
                <div style={{display:"flex",gap:10,justifyContent:"center",marginTop:20,flexWrap:"wrap"}}>
                  <button onClick={()=>setView("create")} style={{background:"var(--gold)",color:"var(--gold-ink)",border:"none",padding:"11px 28px",borderRadius:5,cursor:"pointer",fontFamily:"'Bebas Neue'",fontSize:10,fontWeight:700,letterSpacing:3,textTransform:"uppercase"}}>CREATE FIRST CARD</button>
                  <button onClick={()=>importRef.current?.click()} style={{background:"none",border:"1px solid color-mix(in srgb, var(--gold) 27%, transparent)",color:"var(--gold)",padding:"11px 24px",borderRadius:5,cursor:"pointer",fontFamily:"'Bebas Neue'",fontSize:10,letterSpacing:3,textTransform:"uppercase"}}>⬆ RESTORE BACKUP</button>
                </div>
          ...
          <div style={{animation:"fadeUp 0.4s ease",maxWidth:540,margin:"0 auto"}}>
            ...
            {step===0&&(
              <div style={{animation:"fadeUp 0.3s ease"}}>
                <div style={{background:"var(--s0f)",border:"1px solid var(--b15)",borderRadius:10,padding:"26px 24px",marginBottom:14}}>
                  ...
                  <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" style={{display:"inline-flex",alignItems:"center",gap:8,background:"#0a66c2",color:"#fff",textDecoration:"none",padding:"9px 18px",borderRadius:5,fontFamily:"'Bebas Neue'",fontSize:9,fontWeight:700,letterSpacing:2,textTransform:"uppercase"}}>OPEN LINKEDIN ↗</a>
                </div>
                ...
                <button onClick={()=>setStep(1)} style={{width:"100%",background:"var(--gold)",color:"var(--gold-ink)",border:"none",padding:"13px",borderRadius:5,cursor:"pointer",fontFamily:"'Bebas Neue'",fontSize:11,fontWeight:700,letterSpacing:3,textTransform:"uppercase"}}>GOT MY SCREENSHOTS →</button>
              </div>
            )}
            ...
            {step===1&&(
              <div style={{animation:"fadeUp 0.3s ease"}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:12}}>
                    {imgs.map((im,i)=>(
                      <div key={i} style={{background:"var(--s0f)",border:"1px solid var(--b15)",borderRadius:8,display:"flex",alignItems:"center",gap:10,padding:"8px 12px"}}>
                        ...
                        <button onClick={()=>setImgs(p=>p.filter((_,j)=>j!==i))} style={{background:"none",border:"none",color:"var(--v333)",cursor:"pointer",fontFamily:"'Bebas Neue'",fontSize:9,padding:0}}>remove</button>
                      </div>
                    ))}
                  </div>
                )}
                <div
                  ...
                  <div style={{color:"color-mix(in srgb, var(--gold) 40%, transparent)",fontSize:11,letterSpacing:1,marginBottom:4,fontFamily:"'Bebas Neue'"}}>{IS_MOBILE?"Tap here to add your screenshots":"Ctrl + V to paste from clipboard"}</div>
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <button className="ghost" onClick={()=>setStep(0)} style={{background:"none",border:"none",color:"var(--v333)",cursor:"pointer",fontFamily:"'Bebas Neue'",fontSize:9,letterSpacing:2,textTransform:"uppercase",padding:0,transition:"color 0.15s"}}>↠BACK</button>
                  {imgs.length>0&&<button onClick={()=>setStep(2)} style={{background:"var(--gold)",color:"var(--gold-ink)",border:"none",padding:"10px 20px",borderRadius:5,cursor:"pointer",fontFamily:"'Bebas Neue'",fontSize:9,fontWeight:700,letterSpacing:2,textTransform:"uppercase"}}>ANALYSE {imgs.length} SCREENSHOT{imgs.length>1?"S":""} →</button>}
                </div>
              </div>
            )}
            ...
            {step===2&&imgs.length>0&&(
              <div style={{animation:"fadeUp 0.3s ease"}}>
                <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:12}}>
                  {imgs.map((im,i)=>(
                    <div key={i} onClick={()=>openLB(imgs.map(x=>x.preview),i,di=>setImgs(p=>p.filter((_,j)=>j!==di)))} title="Click to inspect full size" style={{background:"var(--s0f)",border:"1px solid var(--b15)",borderRadius:8,overflow:"hidden",cursor:"pointer",position:"relative"}}>
                      ...
                      <div style={{position:"absolute",right:8,bottom:6,background:"rgba(0,0,0,0.55)",color:"#fff",fontSize:8,letterSpacing:1,padding:"3px 8px",borderRadius:3,fontFamily:"'Bebas Neue',sans-serif"}}>🔍 CLICK TO INSPECT</div>
                    </div>
                  ))}
                  <button onClick={()=>setStep(1)} style={{background:"none",border:"none",color:"var(--v333)",cursor:"pointer",fontFamily:"'Bebas Neue'",fontSize:9,padding:0,textDecoration:"underline",alignSelf:"flex-end"}}>edit screenshots</button>
                </div>
                {dupWarn&&(
                  <div style={{background:"var(--warn-bg)",border:"1px solid color-mix(in srgb, var(--c-reach) 20%, transparent)",borderRadius:8,padding:"14px 18px",marginBottom:14}}>
                    ...
                    <div style={{display:"flex",gap:8}}>
                      <button onClick={()=>{setUpdating(dupWarn.id);setDupWarn(null);analyse(true);}} style={{background:"var(--c-reach)",color:"var(--gold-ink)",border:"none",padding:"8px 16px",borderRadius:4,cursor:"pointer",fontFamily:"'Bebas Neue'",fontSize:9,fontWeight:700,letterSpacing:1,textTransform:"uppercase"}}>UPDATE CARD</button>
                      <button onClick={()=>{setDupWarn(null);reset();}} style={{background:"none",border:"1px solid var(--v333)",color:"var(--v555)",padding:"8px 16px",borderRadius:4,cursor:"pointer",fontFamily:"'Bebas Neue'",fontSize:9,letterSpacing:1,textTransform:"uppercase"}}>SKIP</button>
                    </div>
                  </div>
                )}
                {(extracting||scoring)&&(
                ...
                {!dupWarn&&<button onClick={()=>analyse(false)} disabled={extracting||scoring} style={{width:"100%",background:extracting||scoring?"var(--s11)":"var(--gold)",color:extracting||scoring?"var(--v333)":"var(--gold-ink)",border:"none",padding:"13px",borderRadius:5,cursor:extracting||scoring?"not-allowed":"pointer",fontFamily:"'Bebas Neue'",fontSize:11,fontWeight:700,letterSpacing:3,textTransform:"uppercase",transition:"background 0.15s"}}>
            ...
            {step===3&&done&&revealed&&(
              <div style={{textAlign:"center",animation:"fadeUp 0.5s ease"}}>
                ...
                {done.history?.length>0&&(()=>{const ps=done.history[done.history.length-1];const d=done.OVR-ps.OVR;return(
                  <div style={{fontSize:10,marginBottom:14,letterSpacing:1,fontFamily:"'Bebas Neue',sans-serif",color:d>0?"#16a34a":d<0?"#dc2626":"var(--v444)"}}>{d===0?"OVR unchanged":(d>0?`▲ OVR +${d}`:`▼ OVR ${d}`)} since last scan ({new Date(ps.date).toLocaleDateString()}){done.lastDeltaCause&&d!==0&&<span style={{display:"block",color:"var(--v444)",fontSize:8,marginTop:3,letterSpacing:0.5}}>why: {done.lastDeltaCause}</span>}</div>
                );})()}
                ...
                {done.roast&&<div style={{background:"var(--warn-bg)",border:"1px solid color-mix(in srgb, var(--c-reach) 33%, transparent)",borderRadius:8,padding:18,marginBottom:14,textAlign:"left"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    ...
                    {!(roastOpen||roastMode)&&<button onClick={()=>setRoastOpen(true)} style={{background:"var(--c-reach)",color:"#fff",border:"none",padding:"6px 14px",borderRadius:4,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:8,fontWeight:700,letterSpacing:1,textTransform:"uppercase"}}>REVEAL</button>}
                  </div>
                ...
                <div style={{display:"flex",gap:8,justifyContent:"center"}}>
                  <button onClick={reset} style={{background:"none",border:"1px solid var(--v1e)",color:"var(--v555)",padding:"10px 20px",borderRadius:5,cursor:"pointer",fontFamily:"'Bebas Neue'",fontSize:9,letterSpacing:2,textTransform:"uppercase"}}>ADD ANOTHER</button>
                  <button onClick={()=>{setSel(done);setView("profile");}} style={{background:"var(--gold)",color:"var(--gold-ink)",border:"none",padding:"10px 22px",borderRadius:5,cursor:"pointer",fontFamily:"'Bebas Neue'",fontSize:9,fontWeight:700,letterSpacing:2,textTransform:"uppercase"}}>VIEW PROFILE →</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{animation:"fadeUp 0.4s ease"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:32}}>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                {ioMsg&&<span style={{color:"var(--v555)",fontSize:9,letterSpacing:1,fontFamily:"'Bebas Neue',sans-serif",marginRight:6}}>{ioMsg}</span>}
                <select value={lbCohort} onChange={e=>setLbCohort(e.target.value)} style={{background:"var(--s0f)",border:"1px solid var(--v1e)",borderRadius:5,padding:"8px 10px",color:"var(--v555)",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,letterSpacing:1,cursor:"pointer",outline:"none"}}>
                  <option value="all">ALL COHORTS</option>
                  {cohorts.map(y=><option key={y} value={y}>CLASS OF {y}</option>)}
                </select>
                <button onClick={exportCards} disabled={!cards.length} style={{background:"none",border:"1px solid var(--v1e)",color:cards.length?"var(--v555)":"var(--v1e)",padding:"8px 14px",borderRadius:5,cursor:cards.length?"pointer":"not-allowed",fontFamily:"'Bebas Neue'",fontSize:9,letterSpacing:2,textTransform:"uppercase"}}>⬇ EXPORT</button>
                <button onClick={()=>importRef.current?.click()} style={{background:"none",border:"1px solid var(--v1e)",color:"var(--v555)",padding:"8px 14px",borderRadius:5,cursor:"pointer",fontFamily:"'Bebas Neue'",fontSize:9,letterSpacing:2,textTransform:"uppercase"}}>⬆ IMPORT</button>
                <button onClick={()=>setView("create")} style={{background:"none",border:"1px solid color-mix(in srgb, var(--gold) 20%, transparent)",color:"var(--gold)",padding:"8px 16px",borderRadius:5,cursor:"pointer",fontFamily:"'Bebas Neue'",fontSize:9,letterSpacing:2,textTransform:"uppercase"}} onMouseEnter={e=>e.target.style.borderColor="color-mix(in srgb, var(--gold) 40%, transparent)"} onMouseLeave={e=>e.target.style.borderColor="color-mix(in srgb, var(--gold) 20%, transparent)"}>+ ADD PROFILE</button>
              </div>
            </div>
            {sorted.length===0?(
          ...
          <div style={{animation:"fadeUp 0.4s ease"}}>
            ...
            {cards.length<2?(
              <div style={{textAlign:"center",padding:"64px 0"}}>
                ...
                <button onClick={()=>setView("create")} style={{background:"var(--gold)",color:"var(--gold-ink)",border:"none",padding:"10px 22px",borderRadius:5,cursor:"pointer",fontFamily:"'Bebas Neue'",fontSize:9,fontWeight:700,letterSpacing:2,textTransform:"uppercase"}}>+ ADD PROFILES</button>
              </div>
            ):(
              <>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:28}}>
                  {[["A",vsA,setVsA,vsB],["B",vsB,setVsB,vsA]].map(([label,val,setVal,other])=>(
                    <select key={label} value={val} onChange={e=>setVal(e.target.value)} style={{background:"var(--s0f)",border:"1px solid var(--b15)",borderRadius:6,padding:"11px 12px",color:val?"var(--vddd)":"var(--v444)",fontFamily:"'Bebas Neue',sans-serif",fontSize:10,letterSpacing:0.5,cursor:"pointer",outline:"none"}}>
                    ...
                    <div style={{animation:"fadeUp 0.3s ease"}}>
                      <div style={{display:"flex",gap:20,justifyContent:"center",alignItems:"center",flexWrap:"wrap",marginBottom:28}}>
                        ...
                        <div style={{textAlign:"center",minWidth:130}}>
                          ...
                          <div style={{color:"var(--v444)",fontSize:10,letterSpacing:1,marginTop:6,fontFamily:"'Bebas Neue',sans-serif"}}>{aWins} – {bWins} on stats</div>
                      ...
                      <div style={{background:"var(--s0f)",border:"1px solid color-mix(in srgb, var(--gold) 27%, transparent)",boxShadow:"0 0 16px color-mix(in srgb, var(--gold) 8%, transparent)",borderRadius:8,padding:"18px 22px",maxWidth:560,margin:"14px auto 0"}}>
                            ...
                            <>
                              ...
                              {rows.map(r=>(
                                <div key={r.s} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderBottom:"1px solid var(--b14)"}}>
                                  <span style={{color:"var(--v666)",fontSize:9,fontFamily:"'Bebas Neue',sans-serif"}}>{r.s} · {r.d>0?an:bn} leads by {Math.abs(r.d)}</span>
                        ...
                        {a.id!=="__field"&&b.id!=="__field"&&(
                          <div style={{marginTop:12}}>
                            {!vsVerdicts[`${a.id}|${b.id}`]&&<button onClick={()=>genVerdict(a,b)} disabled={vsBusy} style={{background:"var(--gold)",color:"var(--gold-ink)",border:"none",padding:"9px 18px",borderRadius:5,cursor:vsBusy?"wait":"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,fontWeight:700,letterSpacing:2,textTransform:"uppercase",opacity:vsBusy?0.6:1}}>{vsBusy?"JUDGING…":"GENERATE SCOUT VERDICT"}</button>}
          ...
          <div style={{animation:"fadeUp 0.4s ease",maxWidth:640,margin:"0 auto"}}>
            ...
            <div onDragOver={e=>{e.preventDefault();setDrag(true);}} onDragLeave={()=>setDrag(false)} onDrop={e=>{e.preventDefault();setDrag(false);const f=e.dataTransfer.files?.[0];if(f&&f.type.startsWith("image/")){const rd=new FileReader();rd.onload=ev=>setHgImg({b64:ev.target.result.split(",")[1],type:f.type||"image/png",preview:ev.target.result});rd.readAsDataURL(f);}}} style={{background:drag?"color-mix(in srgb, var(--gold) 3%, transparent)":"var(--s0f)",border:`2px dashed ${drag?"var(--gold)":"color-mix(in srgb, var(--gold) 27%, transparent)"}`,boxShadow:"0 0 18px color-mix(in srgb, var(--gold) 8%, transparent)",borderRadius:10,padding:"20px 22px",marginBottom:14,transition:"all 0.2s"}}>
              <div style={{textAlign:"center",marginBottom:12}}>
                <div style={{color:"color-mix(in srgb, var(--gold) 60%, transparent)",fontSize:11,letterSpacing:1,fontFamily:"'Bebas Neue',sans-serif"}}>{IS_MOBILE?"Tap to add a screenshot of the post":"Ctrl + V anywhere to paste a screenshot of the post"}</div>
                <div style={{color:"var(--v333)",fontSize:9,letterSpacing:1,marginTop:2}}>or drop an image here · or type / paste the text below</div>
              </div>
              <textarea value={hgText} onChange={e=>setHgText(e.target.value)} onPaste={e=>{const items=e.clipboardData?.items;if(!items)return;for(const it of items){if(it.type.startsWith("image/")){const f=it.getAsFile();const rd=new FileReader();rd.onload=ev=>setHgImg({b64:ev.target.result.split(",")[1],type:f.type||"image/png",preview:ev.target.result});rd.readAsDataURL(f);e.preventDefault();break;}}}} placeholder={"Paste the post text — or paste / upload a screenshot of it…\ne.g. \"Thrilled to announce I'll be joining X as a Y this summer…\""} style={{width:"100%",minHeight:96,background:"var(--s11)",border:"1px solid var(--v1e)",borderRadius:6,padding:"12px 14px",color:"var(--veee)",fontFamily:"'Bebas Neue',sans-serif",fontSize:10,lineHeight:1.7,outline:"none",resize:"vertical",boxSizing:"border-box",marginBottom:10}}/>
              {hgImg&&(
                <div style={{display:"flex",alignItems:"center",gap:10,background:"var(--s11)",border:"1px solid var(--v1e)",borderRadius:6,padding:"8px 12px",marginBottom:10}}>
                  ...
                  <button onClick={()=>setHgImg(null)} style={{background:"none",border:"none",color:"var(--v333)",cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,padding:0}}>remove</button>
                </div>
              )}
              <input value={hgCtx} onChange={e=>setHgCtx(e.target.value)} placeholder="Optional context — who is this person? e.g. first-year at a non-target, career switcher, first in family at uni…" style={{width:"100%",background:"var(--s11)",border:"1px solid var(--v1e)",borderRadius:6,padding:"10px 12px",color:"var(--veee)",fontFamily:"'Bebas Neue',sans-serif",fontSize:10,outline:"none",boxSizing:"border-box",marginBottom:12}}/>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                <button onClick={()=>hgFileRef.current?.click()} style={{background:"none",border:"1px solid var(--v1e)",color:"var(--v555)",padding:"10px 16px",borderRadius:5,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,letterSpacing:1,textTransform:"uppercase"}}>+ SCREENSHOT</button>
                <button onClick={rateHowGood} disabled={hgBusy||(!hgText.trim()&&!hgImg)} style={{flex:1,minWidth:160,background:hgBusy||(!hgText.trim()&&!hgImg)?"var(--s11)":"var(--gold)",color:hgBusy||(!hgText.trim()&&!hgImg)?"var(--v444)":"var(--gold-ink)",border:"none",padding:"12px 20px",borderRadius:5,cursor:hgBusy?"wait":"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:10,fontWeight:700,letterSpacing:3,textTransform:"uppercase"}}>{hgBusy?`RATING… ${elapsed||""}${elapsed?"s":""}`:"HOW GOOD IS IT?"}</button>
                {hgBusy&&<button onClick={cancelActiveCall} style={{background:"none",border:"1px solid var(--v1e)",color:"var(--v555)",padding:"10px 14px",borderRadius:5,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:8,letterSpacing:1,textTransform:"uppercase"}}>CANCEL</button>}
              </div>
                ...
                <div style={{background:"var(--s0f)",border:`1px solid ${A(gradeColor,40)}`,boxShadow:`0 0 24px ${A(gradeColor,13)}`,borderRadius:10,padding:"22px 24px",marginBottom:10}}>
                  <div style={{display:"flex",gap:18,alignItems:"center",flexWrap:"wrap"}}>
                    <div style={{width:74,height:74,borderRadius:12,background:A(gradeColor,13),border:`2px solid ${A(gradeColor,53)}`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                      ...
                      <div style={{color:"var(--v444)",fontSize:8,letterSpacing:1,fontFamily:"'Bebas Neue',sans-serif"}}>{hgResult.score}/99</div>
                    </div>
                    ...
                    <div style={{marginTop:12}}>
                      ...
                      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8}}>
                        {["Is this actually rare or does it just sound rare?","What would the S-tier version of this be?","How should they leverage this next?"].map(q=>(
                          <button key={q} onClick={()=>sendHgChat(q)} disabled={hgChatBusy} style={{background:"none",border:"1px solid color-mix(in srgb, var(--gold) 22%, transparent)",color:"var(--v666)",padding:"5px 10px",borderRadius:12,cursor:hgChatBusy?"wait":"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:8,letterSpacing:0.5}}>{q}</button>
                        ))}
                      </div>
                      <div style={{display:"flex",gap:8}}>
                        <input value={hgChatIn} onChange={e=>setHgChatIn(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")sendHgChat();}} placeholder="Ask a follow-up about this achievement…" style={{flex:1,background:"var(--s11)",border:"1px solid var(--v1e)",borderRadius:6,padding:"10px 12px",color:"var(--veee)",fontFamily:"'Bebas Neue',sans-serif",fontSize:10,outline:"none"}}/>
                        <button onClick={()=>sendHgChat()} disabled={hgChatBusy||!hgChatIn.trim()} style={{background:hgChatBusy||!hgChatIn.trim()?"var(--s11)":"var(--gold)",color:hgChatBusy||!hgChatIn.trim()?"var(--v444)":"var(--gold-ink)",border:"none",padding:"10px 18px",borderRadius:6,cursor:hgChatBusy?"wait":"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,fontWeight:700,letterSpacing:2,textTransform:"uppercase"}}>SEND</button>
                      </div>
                    </div>
                  )}
                </div>
                <div style={{display:"flex",gap:8,justifyContent:"center",marginTop:6}}>
                  <button onClick={()=>{setHgResult(null);setHgText("");setHgImg(null);setHgCtx("");setHgChat([]);setHgChatOpen(false);}} style={{background:"none",border:"1px solid var(--v1e)",color:"var(--v555)",padding:"9px 18px",borderRadius:5,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,letterSpacing:2,textTransform:"uppercase"}}>RATE ANOTHER</button>
                </div>
              </div>
              );
            })()}
          ...
          <div style={{animation:"fadeUp 0.4s ease",maxWidth:640,margin:"0 auto"}}>
              ...
              <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",zIndex:250,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
                ...
                {!rsReveal.open?(
                  <>
                    ...
                    <div style={{color:"#bbbbbb",fontSize:9,letterSpacing:2,marginTop:16,textTransform:"uppercase",fontFamily:"'Bebas Neue',sans-serif"}}>tap to reveal the upgraded card</div>
                  </>
                ):(
            ...
            {(sel.rubricV||0)<RUBRIC_VERSION&&!rescoring&&(
              <div style={{background:"var(--warn-bg)",border:"1px solid color-mix(in srgb, var(--c-reach) 33%, transparent)",borderRadius:8,padding:"10px 14px",marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                ...
                <button onClick={rescore} disabled={rescoring} style={{background:"var(--c-reach)",color:"#fff",border:"none",padding:"8px 16px",borderRadius:4,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:8,fontWeight:700,letterSpacing:1,textTransform:"uppercase"}}>RE-SCORE NOW</button>
              </div>
            )}
            {rescoring&&(
              ...
              <div style={{flex:1,minWidth:200,display:"flex",flexDirection:"column",gap:14}}>
                  ...
                  {STATS.map(st=>{
                    ...
                    return(
                      ...
                      <div onClick={()=>{if(hasReason)setOpenStats(o=>({...o,[st]:!o[st]}));}} style={{display:"flex",alignItems:"center",gap:8,cursor:hasReason?"pointer":"default"}}>
                        ...
                        {(()=>{const ps=sel.history?.[sel.history.length-1];if(!ps?.stats)return null;const d=v-(ps.stats[st]??v);if(!d)return null;return <span style={{color:d>0?"#16a34a":"#dc2626",fontSize:8,minWidth:22,fontFamily:"'Bebas Neue',sans-serif"}}>{d>0?`+${d}`:d}</span>;})()}
                ...
                <div style={{background:"var(--s0f)",border:`1px solid ${ct.acc}55`,boxShadow:`0 0 20px ${ct.acc}22`,borderRadius:8,padding:"16px 18px",textAlign:"center"}}>
                  ...
                  {sel.history?.length>0&&(()=>{const ps=sel.history[sel.history.length-1];const d=sel.OVR-ps.OVR;return(
                    <div style={{fontSize:9,marginTop:8,letterSpacing:1,fontFamily:"'Bebas Neue',sans-serif",color:d>0?"#16a34a":d<0?"#dc2626":"var(--v444)"}}>{d===0?"unchanged":(d>0?`▲ +${d}`:`▼ ${d}`)} since last scan · {new Date(ps.date).toLocaleDateString()}{sel.lastDeltaCause&&d!==0&&<span style={{display:"block",color:"var(--v444)",fontSize:8,marginTop:3,letterSpacing:0.5}}>why: {sel.lastDeltaCause}</span>}</div>
                  );})()}
              ...
              return(
                <div style={{background:"var(--s0f)",border:"1px solid color-mix(in srgb, var(--gold) 33%, transparent)",boxShadow:"0 0 18px color-mix(in srgb, var(--gold) 10%, transparent)",borderRadius:8,padding:"18px 20px",marginBottom:10}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10,marginBottom:12}}>
                    <div style={{color:"var(--gold)",fontFamily:"'Bebas Neue'",fontSize:16,letterSpacing:2}}>VS THE POOL <span style={{color:"var(--v555)",fontSize:9,fontFamily:"'Bebas Neue',sans-serif",letterSpacing:1}}>· ranked against {cards.length-1} other card{cards.length===2?"":"s"}</span></div>
                    <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                      <button onClick={()=>{setVsA(sel.id);setVsB("__field");setView("versus");}} style={{background:"var(--gold)",color:"var(--gold-ink)",border:"none",padding:"7px 14px",borderRadius:4,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:8,fontWeight:700,letterSpacing:1,textTransform:"uppercase"}}>FULL MATCHUP VS THE FIELD →</button>
                      <select defaultValue="" onChange={e=>{if(e.target.value){setVsA(sel.id);setVsB(e.target.value);setView("versus");}}} style={{background:"var(--s11)",border:"1px solid var(--v1e)",borderRadius:4,padding:"7px 10px",color:"var(--v555)",fontFamily:"'Bebas Neue',sans-serif",fontSize:8,letterSpacing:1,cursor:"pointer",outline:"none",textTransform:"uppercase"}}>
                      ...
                      return(
                        <div key={r.k} style={{background:"var(--s0c)",border:"1px solid var(--b14)",borderRadius:6,padding:"9px 11px"}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline"}}>
                            <span style={{color:r.color,fontSize:8,letterSpacing:1,fontFamily:"'Bebas Neue',sans-serif"}}>{r.k}</span>
                            <span style={{fontFamily:"'Bebas Neue'",fontSize:16,color:"var(--vddd)"}}>{r.v}</span>
                          </div>
                          <div style={{display:"flex",justifyContent:"space-between",marginTop:3}}>
                            <span style={{fontSize:8,color:"var(--v444)",fontFamily:"'Bebas Neue',sans-serif"}}>pool {r.a.toFixed(0)}</span>
                            <span style={{fontSize:9,fontFamily:"'Bebas Neue',sans-serif",color:d>0?"#16a34a":d<0?"#dc2626":"var(--v444)"}}>{d>0?`+${d.toFixed(0)}`:d.toFixed(0)}</span>
                          </div>
                          <div style={{fontSize:8,color:"var(--v444)",marginTop:2,fontFamily:"'Bebas Neue',sans-serif"}}>#{rk} of {cards.length}</div>
                        </div>
                      );
                    })}
                  </div>
            ...
            {myId===sel.id&&(
              <div style={{background:"var(--s0f)",border:"1px solid var(--b15)",borderRadius:8,padding:"14px 18px",marginBottom:10}}>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  <select value={sel.asp_type||""} onChange={e=>saveAsp("asp_type",e.target.value)} style={{background:"var(--s11)",border:"1px solid var(--v1e)",borderRadius:5,padding:"8px 10px",color:sel.asp_type?"var(--vddd)":"var(--v444)",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,letterSpacing:0.5,cursor:"pointer",outline:"none"}}>
                    <option value="">— aspirational type —</option>
                    {PROFILE_TYPES.map(t=><option key={t} value={t}>{t}</option>)}
                  </select>
                  <select value={sel.asp_build||""} onChange={e=>saveAsp("asp_build",e.target.value)} style={{background:"var(--s11)",border:"1px solid var(--v1e)",borderRadius:5,padding:"8px 10px",color:sel.asp_build?"var(--vddd)":"var(--v444)",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,letterSpacing:0.5,cursor:"pointer",outline:"none"}}>
            ...
            <div style={{background:"var(--s0f)",border:`1px solid ${deepDive?"color-mix(in srgb, var(--gold) 33%, transparent)":"var(--b15)"}`,boxShadow:deepDive?"0 0 18px color-mix(in srgb, var(--gold) 10%, transparent)":"none",borderRadius:8,marginBottom:10}}>
              <div onClick={()=>setDeepDive(d=>!d)} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"15px 18px",cursor:"pointer",userSelect:"none"}}>
                <div><span style={{color:"var(--gold)",fontFamily:"'Bebas Neue'",fontSize:16,letterSpacing:2}}>DEEP DIVE ANALYSIS</span><span style={{color:"var(--v555)",fontSize:9,marginLeft:10,fontFamily:"'Bebas Neue',sans-serif",letterSpacing:0.5}}>· the full rationale behind the numbers</span></div>
                        ...
                        {open&&(
                          <div style={{padding:"0 0 12px 16px"}}>
                            {note
                              ...
                              :<button onClick={()=>genRefWhy(r)} disabled={refBusy} style={{background:"var(--gold)",color:"var(--gold-ink)",border:"none",padding:"8px 14px",borderRadius:4,cursor:refBusy?"wait":"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:8,fontWeight:700,letterSpacing:1,textTransform:"uppercase",marginBottom:8,opacity:refBusy?0.6:1}}>{refBusy?"ANALYSING…":"WHY THIS REFERENCE?"}</button>}
                            {refErr&&<div style={{color:"#ff4444",fontSize:9,marginBottom:8}}>{refErr}</div>}
                            <button onClick={()=>jump(r)} style={{background:"none",border:"1px solid var(--v1e)",color:"var(--v555)",padding:"7px 12px",borderRadius:4,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:8,letterSpacing:1,textTransform:"uppercase"}}>OPEN THEIR CARD →</button>
                          </div>
                        )}
                      </div>
                    );})}
            ...
            {myId===sel.id&&(
              <div style={{background:"var(--s0f)",border:"1px solid color-mix(in srgb, var(--gold) 33%, transparent)",boxShadow:"0 0 18px color-mix(in srgb, var(--gold) 10%, transparent)",borderRadius:8,padding:"18px 20px",marginTop:10}}>
                ...
                <select defaultValue="" onChange={e=>{if(e.target.value){setBenchList([...benchIds,e.target.value]);e.target.value="";}}} style={{width:"100%",background:"var(--s11)",border:"1px solid var(--v1e)",borderRadius:6,padding:"9px 12px",color:"var(--vddd)",fontFamily:"'Bebas Neue',sans-serif",fontSize:10,outline:"none",marginBottom:10,cursor:"pointer"}}>
                    ...
                    <div key={id} style={{borderTop:"1px solid var(--b14)",padding:"10px 0"}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:6}}>
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:10}}>
                          ...
                          {move!==null&&move!==0&&<span style={{fontSize:8,color:move>0?"#dc2626":"#16a34a",fontFamily:"'Bebas Neue',sans-serif"}}>{move>0?`gap widened +${move}`:`gap closed ${-move}`}</span>}
                          <button onClick={()=>genVerdict(sel,b)} disabled={vsBusy} style={{background:"none",border:"1px solid color-mix(in srgb, var(--gold) 27%, transparent)",color:"var(--gold)",padding:"4px 10px",borderRadius:4,cursor:vsBusy?"wait":"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:8,letterSpacing:1,textTransform:"uppercase"}}>{vsBusy?"…":"ANALYSE GAP"}</button>
                          <button onClick={()=>setBenchList(benchIds.filter(x=>x!==id))} style={{background:"none",border:"none",color:"var(--v333)",cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,padding:0}}>✕</button>
                        </div>
                      </div>
                      <div style={{display:"flex",gap:10,flexWrap:"wrap",marginTop:6}}>
                        {STATS.map(s=>{const d=b.stats[s]-sel.stats[s];return <span key={s} style={{fontSize:8,color:"var(--v555)",fontFamily:"'Bebas Neue',sans-serif"}}>{s} <span style={{color:d>0?"var(--c-reach)":d<0?"#16a34a":"var(--v444)"}}>{d>0?`-${d}`:d<0?`+${-d}`:"="}</span></span>;})}
                      </div>
            ...
            {myId===sel.id&&(
              <div style={{background:"var(--s0f)",border:"1px solid color-mix(in srgb, var(--gold) 33%, transparent)",boxShadow:"0 0 18px color-mix(in srgb, var(--gold) 10%, transparent)",borderRadius:8,padding:"18px 20px",marginTop:10}}>
                <div style={{color:"var(--gold)",fontFamily:"'Bebas Neue'",fontSize:16,letterSpacing:2,marginBottom:4}}>SIGNAL PLAN <span style={{color:"var(--v555)",fontSize:9,fontFamily:"'Bebas Neue',sans-serif",letterSpacing:1}}>· time-aware · checkable</span></div>
                ...
                <textarea key={sel.id} ref={goalsRef} defaultValue={sel.goals||""} placeholder="Your goals — e.g. growth role at a high-traction startup by summer 2027; build j5studies to 100K…" style={{width:"100%",minHeight:56,background:"var(--s11)",border:"1px solid var(--v1e)",borderRadius:6,padding:"10px 12px",color:"var(--veee)",fontFamily:"'Bebas Neue',sans-serif",fontSize:10,lineHeight:1.6,outline:"none",resize:"vertical",boxSizing:"border-box",marginBottom:10}}/>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
                  <button onClick={genPlan} disabled={planBusy} style={{background:"var(--gold)",color:"var(--gold-ink)",border:"none",padding:"9px 18px",borderRadius:5,cursor:planBusy?"wait":"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,fontWeight:700,letterSpacing:2,textTransform:"uppercase",opacity:planBusy?0.6:1}}>{planBusy?"PLANNING…":(sel.plan?"REFRESH PLAN":"GENERATE PLAN")}</button>
                  ...
                  <div key={i} style={{borderTop:"1px solid var(--b14)",padding:"10px 0",opacity:it.status==="na"?0.45:1}}>
                    <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"flex-start"}}>
                      ...
                      <div style={{display:"flex",gap:6,flexShrink:0}}>
                        <button onClick={()=>setPlanStatus(i,"done")} title="Mark done (click again to undo)" style={{background:it.status==="done"?"#16a34a":"none",color:it.status==="done"?"#fff":"var(--v444)",border:"1px solid "+(it.status==="done"?"#16a34a":"var(--v1e)"),borderRadius:4,padding:"4px 9px",cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9}}>✓</button>
                        <button onClick={()=>setPlanStatus(i,"na")} title="Not eligible / window passed (click again to undo)" style={{background:it.status==="na"?"var(--v333)":"none",color:it.status==="na"?"var(--bg)":"var(--v444)",border:"1px solid var(--v1e)",borderRadius:4,padding:"4px 9px",cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9}}>✕</button>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div style={{background:"var(--s0f)",border:"1px solid color-mix(in srgb, var(--gold) 33%, transparent)",boxShadow:"0 0 18px color-mix(in srgb, var(--gold) 10%, transparent)",borderRadius:8,padding:"18px 20px",marginTop:10}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
                <div style={{color:"var(--gold)",fontFamily:"'Bebas Neue'",fontSize:16,letterSpacing:2}}>MAX PROJECTION <span style={{color:"var(--v555)",fontSize:9,fontFamily:"'Bebas Neue',sans-serif",letterSpacing:1}}>· HYPOTHETICAL</span></div>
                <button onClick={genNinety} disabled={ninetyBusy} style={{background:sel.ninety?"none":"var(--gold)",color:sel.ninety?"var(--v555)":"var(--gold-ink)",border:sel.ninety?"1px solid var(--v1e)":"none",padding:"8px 16px",borderRadius:4,cursor:ninetyBusy?"wait":"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:8,fontWeight:700,letterSpacing:1,textTransform:"uppercase",opacity:ninetyBusy?0.6:1}}>{ninetyBusy?"PROJECTING…":sel.ninety?"REGENERATE":"GENERATE MAX VERSION"}</button>
              </div>
                  ...
                  {STATS.map(st=>{
                    ...
                    return(
                      <div key={st} style={{borderBottom:"1px solid var(--b14)",padding:"7px 0"}}>
                        <div style={{display:"flex",alignItems:"center",gap:10}}>
                          <span style={{color:info.color,fontSize:8,letterSpacing:1,minWidth:40,fontFamily:"'Bebas Neue',sans-serif"}}>{st}</span>
                          ...
                          <span style={{fontSize:9,fontFamily:"'Bebas Neue',sans-serif",color:d>0?"#16a34a":"var(--v444)"}}>{d>0?`+${d}`:d===0?"held":d}</span>
                        </div>
                  ...
                  <div style={{marginTop:12,background:"var(--s0c)",border:"1px solid var(--b14)",borderRadius:8,padding:"14px 16px"}}>
                    ...
                    {sel.ninety.milestones.map((m,i)=>(
                      <div key={i} style={{display:"flex",gap:12,alignItems:"flex-start",padding:"7px 0",borderBottom:i<sel.ninety.milestones.length-1?"1px solid var(--b14)":"none"}}>
                        ...
                        {m.eta&&<div style={{flexShrink:0,background:"var(--s11)",border:"1px solid var(--v1e)",borderRadius:4,padding:"3px 9px",color:"var(--gold)",fontSize:8,letterSpacing:1,fontFamily:"'Bebas Neue',sans-serif",textTransform:"uppercase"}}>ETA {m.eta}</div>}
                      </div>
                    ))}
            ...
            <div style={{background:"var(--s0f)",border:"1px solid color-mix(in srgb, var(--gold) 33%, transparent)",boxShadow:"0 0 18px color-mix(in srgb, var(--gold) 10%, transparent)",borderRadius:8,padding:"18px 20px",marginTop:10}}>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>
                {["Why is this profile not rated higher?","What's the strongest case against this rating?","What's the fastest realistic +5 OVR?","Is anything here likely overstated — or understated?"].map(q=>(
                  <button key={q} onClick={()=>sendChat(q)} disabled={chatBusy} style={{background:"none",border:"1px solid color-mix(in srgb, var(--gold) 22%, transparent)",color:"var(--v666)",padding:"6px 10px",borderRadius:12,cursor:chatBusy?"wait":"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:8,letterSpacing:0.5}}>{q}</button>
                ))}
              </div>
              <div style={{display:"flex",gap:8}}>
                <input value={chatIn} onChange={e=>setChatIn(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")sendChat();}} placeholder="e.g. What's realistically holding the OVR back?" style={{flex:1,background:"var(--s11)",border:"1px solid var(--v1e)",borderRadius:6,padding:"11px 14px",color:"var(--veee)",fontFamily:"'Bebas Neue',sans-serif",fontSize:10,outline:"none"}}/>
            ...
            <div style={{background:"var(--s0f)",border:"1px solid color-mix(in srgb, var(--gold) 33%, transparent)",boxShadow:"0 0 18px color-mix(in srgb, var(--gold) 10%, transparent)",borderRadius:8,padding:"18px 20px",marginTop:10}}>
              ...
              {postImg&&(
                <div style={{display:"flex",alignItems:"center",gap:10,background:"var(--s11)",border:"1px solid var(--v1e)",borderRadius:6,padding:"8px 12px",marginBottom:10}}>
                  ...
                  <button onClick={()=>setPostImg(null)} style={{background:"none",border:"none",color:"var(--v333)",cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,padding:0}}>remove</button>
                </div>
              )}
              ...
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <button onClick={()=>postFileRef.current?.click()} style={{background:"none",border:"1px solid var(--v1e)",color:"var(--v555)",padding:"9px 14px",borderRadius:5,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,letterSpacing:1,textTransform:"uppercase"}}>+ SCREENSHOT</button>
                <button onClick={analysePost} disabled={postBusy||(!postIn.trim()&&!postImg)} style={{background:postBusy||(!postIn.trim()&&!postImg)?"var(--s11)":"var(--gold)",color:postBusy||(!postIn.trim()&&!postImg)?"var(--v444)":"var(--gold-ink)",border:"none",padding:"9px 18px",borderRadius:5,cursor:postBusy?"wait":"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,fontWeight:700,letterSpacing:2,textTransform:"uppercase"}}>{postBusy?"READING…":"ANALYSE POST"}</button>
        ...
        {view==="guide"&&(
          <div style={{animation:"fadeUp 0.4s ease",maxWidth:660,margin:"0 auto"}}>
            ...
            <div style={{background:"var(--s0f)",border:"1px solid color-mix(in srgb, var(--gold) 33%, transparent)",boxShadow:"0 0 18px color-mix(in srgb, var(--gold) 10%, transparent)",borderRadius:8,padding:"20px 22px",marginTop:14}}>
              ...
              <textarea value={knowledge} onChange={e=>setKnowledge(e.target.value)} placeholder={"e.g. URSS is a competitive funded research scheme at Warwick. A UK 'spring week' is a selective first-year insight programme at a bank. WFS exec roles are elected, not appointed."} style={{width:"100%",minHeight:120,background:"var(--s11)",border:"1px solid var(--v1e)",borderRadius:6,padding:"12px 14px",color:"var(--veee)",fontFamily:"'Bebas Neue',sans-serif",fontSize:10,lineHeight:1.7,outline:"none",resize:"vertical",boxSizing:"border-box"}}/>
              <div style={{display:"flex",alignItems:"center",gap:10,marginTop:10}}>
                <button onClick={saveKnowledge} style={{background:"var(--gold)",color:"var(--gold-ink)",border:"none",padding:"9px 18px",borderRadius:5,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,fontWeight:700,letterSpacing:2,textTransform:"uppercase"}}>SAVE</button>
            ...
            <div style={{background:"var(--s0f)",border:"1px solid color-mix(in srgb, var(--gold) 33%, transparent)",boxShadow:"0 0 18px color-mix(in srgb, var(--gold) 10%, transparent)",borderRadius:8,padding:"20px 22px",marginTop:14}}>
              ...
              <textarea value={rubric} onChange={e=>setRubric(e.target.value)} placeholder={"e.g. Shipped products with real users beat any society title. A spring week is table stakes, not impressive. 'Founder' of a landing page is theatre; 'founder' with revenue is cracked. Conversions (spring to summer, return offers) are the strongest quiet signal."} style={{width:"100%",minHeight:120,background:"var(--s11)",border:"1px solid var(--v1e)",borderRadius:6,padding:"12px 14px",color:"var(--veee)",fontFamily:"'Bebas Neue',sans-serif",fontSize:10,lineHeight:1.7,outline:"none",resize:"vertical",boxSizing:"border-box"}}/>
              <div style={{display:"flex",alignItems:"center",gap:10,marginTop:10}}>
                <button onClick={saveRubric} style={{background:"var(--gold)",color:"var(--gold-ink)",border:"none",padding:"9px 18px",borderRadius:5,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:9,fontWeight:700,letterSpacing:2,textTransform:"uppercase"}}>SAVE</button>
